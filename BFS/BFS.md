...
分析：帮助完小帅之后，也要为我们的offer好好准备下了，面试算法很少会直接出一个图的bfs题型，比较常见的是这种题型。这题看似和bfs没有关系，起点hit能匹配的字符串为*it，h * t，hi*，与worldList中可以匹配的节点就可以连接了，这样依次连接下去是不是就可以练成图了！这题也就变成了求hit到cog的最短距离。
